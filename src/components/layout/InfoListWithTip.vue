...
<template>
    <div
      class="info-block flex flex-col items-center gap-[20px] pb-[20px] text-start"
    >
      ...
      <ul class="info-list flex flex-col gap-[10px]">
        <li
          ...
          :key="index"
          class="flex items-start gap-[7.5px] list-img"
        >
          <span class="pt-[2.5px] icon">
            <svg
              width="16"
              height="16"
              viewBox="0 0 16 16"
      ...
      <p v-if="listDescription" class="text-base text-content">
        {{ listDescription }}
      </p>
    </div>
  </template>
  ...
  .info-block {
    ...
    &::after{ 
            content: '';
            background-image: url(../../assets/img/tips-n/bg-items.png);
            position: absolute;
            top: 0;
            bottom: auto;
            left: 0;
